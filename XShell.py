...
# Check Device Connect the internet
def checkConnection():
    global connection
    connection = False
    try:
        connect = requests.get('https://github.com/Zrexer')
        
        if connect:
            connection = True
            print('[+] System is Connect to Internet')
            print('[+] Now You can use ChatGPT')
    
    except:
        print('[-] System is Cannot Connect to Internet')
        print('[-] So You cannot use ChatGPT: --gpt')
        
def chatGPT(q):
    return str(dict(requests.get(f'https://haji-api.ir/Free-GPT3/?text={q}&key=hajiapi').json()).get('result').get('answer'))
...
def windowsShell():
    while 1:
        try:
            path = os.getcwd()
            user = str(input(f'\033[95m<\033[96mXSHell\033[95m> \033[93m[ \033[97m{path}\033[93m ]\033[97m> '))
            ...
            elif "--gpt" in user.split():
                if connection == True:
                    text = str(input("[+] Enter Your Question: "))
                    print("[+] ",chatGPT(text.replace(" ", "+")))
                    pass
                else:
                    print("[-] Sorry Your system is not Conncet to Internet, if you want to try , type ' --check-connection '")
                    pass
                
            elif "--check-connection" in user.split():
                checkConnection()
                pass
